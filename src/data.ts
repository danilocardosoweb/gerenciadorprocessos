...
export const initialNodes: Node[] = [
  // ROOT
  { id: 'root', type: 'mindmap', position: { x: 0, y: 0 }, data: { label: 'POP — Usinagem de Perfis de Alumínio', nodeType: 'root', requiredIATF: '4.4', category: 'root' } },

  // BRANCHES
  { id: 'inputs',    type: 'mindmap', position: { x: 0, y: 0 }, data: { label: 'Entradas',              nodeType: 'inputs',    category: 'inputs' } },
  { id: 'outputs',   type: 'mindmap', position: { x: 0, y: 0 }, data: { label: 'Saídas',                nodeType: 'outputs',   category: 'outputs' } },
  { id: 'resources', type: 'mindmap', position: { x: 0, y: 0 }, data: { label: 'Recursos / Com o quê?', nodeType: 'resources', category: 'resources', requiredIATF: '7.1.3' } },
  { id: 'people',    type: 'mindmap', position: { x: 0, y: 0 }, data: { label: 'Pessoas / Quem?',       nodeType: 'people',    category: 'people',    requiredIATF: '7.2.1' } },
  { id: 'methods',   type: 'mindmap', position: { x: 0, y: 0 }, data: { label: 'Métodos / Como?',       nodeType: 'methods',   category: 'methods',   requiredIATF: '8.5' } },
  { id: 'kpis',      type: 'mindmap', position: { x: 0, y: 0 }, data: { label: 'Indicadores / Medições',nodeType: 'kpis',      category: 'kpis',      requiredIATF: '9.1.1' } },
  { id: 'safety',    type: 'mindmap', position: { x: 0, y: 0 }, data: { label: 'Segurança do Trabalho', nodeType: 'methods',   category: 'methods' } },
  { id: 'quality',   type: 'mindmap', position: { x: 0, y: 0 }, data: { label: 'Controle de Qualidade', nodeType: 'kpis',      category: 'kpis',      requiredIATF: '9.1.1' } },

  // INPUTS CHILDREN — Seção 5 e 6 do POP
  { id: 'in_op',     type: 'mindmap', position: { x: 0, y: 0 }, data: { label: 'Ordem de Produção (OP) via MRP', category: 'inputs', requiredIATF: '8.2.3' } },
  { id: 'in_mat',    type: 'mindmap', position: { x: 0, y: 0 }, data: { label: 'Perfis de Alumínio (MP conferida)', category: 'inputs' } },
  { id: 'in_draw',   type: 'mindmap', position: { x: 0, y: 0 }, data: { label: 'Desenho Técnico (medidas, furações, tolerâncias)', category: 'inputs', requiredIATF: '8.2.3' } },
  { id: 'in_prog',   type: 'mindmap', position: { x: 0, y: 0 }, data: { label: 'Programa CNC selecionado', category: 'inputs' } },

  // OUTPUTS CHILDREN — Seção 11 do POP
  { id: 'out_ok',    type: 'mindmap', position: { x: 0, y: 0 }, data: { label: 'Peças Boas Identificadas', category: 'outputs', requiredIATF: '8.6' } },
  { id: 'out_nc',    type: 'mindmap', position: { x: 0, y: 0 }, data: { label: 'Peças de Retrabalho Segregadas', category: 'outputs', requiredIATF: '8.7' } },
  { id: 'out_scrap', type: 'mindmap', position: { x: 0, y: 0 }, data: { label: 'Sucata / Cavaco Descartados', category: 'outputs', requiredIATF: '8.7' } },
  { id: 'out_apo',   type: 'mindmap', position: { x: 0, y: 0 }, data: { label: 'Apontamento de Produção Registrado', category: 'outputs' } },

  // RESOURCES CHILDREN — Seção 2 e 7 do POP
  { id: 'res_serra_auto',  type: 'mindmap', position: { x: 0, y: 0 }, data: { label: 'Serra Automática 1 cabeça (Emmegi)', category: 'resources', requiredIATF: '8.5.1.5' } },
  { id: 'res_serra_dupla', type: 'mindmap', position: { x: 0, y: 0 }, data: { label: 'Serra Dupla 2 cabeças (Doppia)', category: 'resources', requiredIATF: '8.5.1.5' } },
  { id: 'res_cnc',         type: 'mindmap', position: { x: 0, y: 0 }, data: { label: 'Centro de Usinagem CNC', category: 'resources', requiredIATF: '8.5.1.5' } },
  { id: 'res_gages',       type: 'mindmap', position: { x: 0, y: 0 }, data: { label: 'Instrumentos de Medição (Calibrados)', category: 'resources', requiredIATF: '7.1.5' } },

  // PEOPLE CHILDREN — Seção 3 do POP
  { id: 'peo_sup',  type: 'mindmap', position: { x: 0, y: 0 }, data: { label: 'Supervisor de Usinagem', category: 'people', requiredIATF: '7.2' } },
  { id: 'peo_op',   type: 'mindmap', position: { x: 0, y: 0 }, data: { label: 'Operador de Usinagem (Treinado)', category: 'people', requiredIATF: '7.2' } },
  { id: 'peo_aux',  type: 'mindmap', position: { x: 0, y: 0 }, data: { label: 'Auxiliar de Produção', category: 'people' } },
  { id: 'peo_qual', type: 'mindmap', position: { x: 0, y: 0 }, data: { label: 'Qualidade / PCP (suporte)', category: 'people' } },

  // METHODS CHILDREN — Seções 7, 8 do POP
  { id: 'met_prep_serra', type: 'mindmap', position: { x: 0, y: 0 }, data: { label: 'Preparação das Serras (disco, lubrificação, batentes)', category: 'methods', requiredIATF: '8.5.1.3' } },
  { id: 'met_prep_cnc',   type: 'mindmap', position: { x: 0, y: 0 }, data: { label: 'Preparação CNC (programa, zeragem, simulação)', category: 'methods', requiredIATF: '8.5.1.3' } },
  { id: 'met_op_serra',   type: 'mindmap', position: { x: 0, y: 0 }, data: { label: 'Operação nas Serras (fixação, corte, aguardar parada)', category: 'methods', requiredIATF: '8.5.1' } },
  { id: 'met_op_cnc',     type: 'mindmap', position: { x: 0, y: 0 }, data: { label: 'Operação no CNC (ciclo automático, monitoramento)', category: 'methods', requiredIATF: '8.5.1' } },
  { id: 'met_nc',         type: 'mindmap', position: { x: 0, y: 0 }, data: { label: 'Tratamento de Não Conformidades (parar, comunicar, registrar)', category: 'methods', requiredIATF: '8.7' } },
  ...
  { id: 'qua_first',  type: 'mindmap', position: { x: 0, y: 0 }, data: { label: 'Medir primeiras peças produzidas', category: 'kpis', requiredIATF: '9.1.1' } },
  { id: 'qua_comp',   type: 'mindmap', position: { x: 0, y: 0 }, data: { label: 'Conferir: Comprimento, Furos, Diâmetros', category: 'kpis', requiredIATF: '9.1.1' } },
  { id: 'qua_draw',   type: 'mindmap', position: { x: 0, y: 0 }, data: { label: 'Comparar com Desenho Técnico', category: 'kpis', requiredIATF: '8.6' } },
  { id: 'qua_seg',    type: 'mindmap', position: { x: 0, y: 0 }, data: { label: 'Segregar e identificar peças fora de especificação', category: 'kpis', requiredIATF: '8.7' } },

  // KPIS CHILDREN — Seção 10 do POP
  { id: 'kpi_prod',   type: 'mindmap', position: { x: 0, y: 0 }, data: { label: 'Quantidade Produzida por OP', category: 'kpis', requiredIATF: '9.1.1.1' } },
  { id: 'kpi_scrap',  type: 'mindmap', position: { x: 0, y: 0 }, data: { label: 'Índice de Refugo e Motivo das Perdas', category: 'kpis', requiredIATF: '9.1.1.1' } },
  { id: 'kpi_stop',   type: 'mindmap', position: { x: 0, y: 0 }, data: { label: 'Paradas de Máquina (setup, manutenção, falta de material)', category: 'kpis', requiredIATF: '8.5.1.5' } },
